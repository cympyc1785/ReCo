...
# ========== Env (W&B) ==========
export RECO_DATA_ROOT=./davis_data
export RECO_EXCLUDE_VAL=0
...
CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --nnodes=$NUM_NODES \
    ...
    --max_steps 2000 \
    --max_epochs 1000 \
    --pretrained_lora_path "checkpoints/ReCo/2026_01_16_v1_release.ckpt" \
    --run_name train_run6_davis_overfit  2>&1 | tee $log_file
    # 如需加载已有 LoRA：
